...
The current vertical slice is tested with a fake inference gateway. That verifies config loading, planning, per-turn Actor/Theater stage dependency gating, transcript assembly, Grader disclosure assessment prompts, assessment aggregation, configured deterministic mutation proposal/application, artifact writes, and completion records without requiring Ollama to be running.
